-----------------------------------------------------------------------------------------------
-- Client Lua Script for CheebaCREDD
...
function CheebaCREDD:Init()
    Apollo.RegisterAddon(self)
end

function CheebaCREDD:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CheebaCREDD.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CheebaCREDD:OnDocumentReady()
...
function CheebaCREDD:OnInterfaceMenuListHasLoaded()
...
function CheebaCREDD:Initialize()
	...
	local wndMain = Apollo.LoadForm(self.xmlDoc, "CheebaCREDDForm", nil, self)
...
function CheebaCREDD:OnToggleCREDDExchangeWindow()
...
function CheebaCREDD:OnClose(wndHandler, wndControl, eMouseButton)
...
function CheebaCREDD:OnOpenListingsBtn(wndHandler, wndControl)
	Event_FireGenericEvent("InterfaceMenu_ToggleCheebaListings")
end

function CheebaCREDD:OnOpenInventoryBtn(wndHandler, wndControl)
...
function CheebaCREDD:OnHeaderLogCheck(wndHandler, wndControl) -- Same Radio Group as OnHeaderTabCheck
	self.tWindowMap["ConfirmationBlocker"]:Show(false)
	CREDDExchangeLib.GetCREDDHistory() -- Leads to OnCREDDExchangeInfoResults
end

function CheebaCREDD:OnHeaderTabCheck(wndHandler, wndControl)
	self.tWindowMap["ConfirmationBlocker"]:Show(false)
	CREDDExchangeLib.RequestExchangeInfo() -- Leads to OnCREDDExchangeInfoResults
end

function CheebaCREDD:OnRefreshMarketBtn(wndHandler, wndControl, eMouseButton)
...
function CheebaCREDD:OnCREDDExchangeInfoResults(arMarketStats, arOrders) -- Main Redraw
	local wndMain = self.tWindowMap["Main"]
	...
	self.tWindowMap["ActLaterPrice"]:SetAmount(bBuyTabChecked and tBuyOrderData.monPrice or tSellOrderData.monPrice)
...
function CheebaCREDD:RefreshEscrow()
	-- Escrow
...
function CheebaCREDD:RefreshBoundCredd()
	local wndMain = self.tWindowMap["Main"]
...
function CheebaCREDD:OnCashInputChanged(wndHandler, wndControl)
	wndHandler:SetText(math.max(0, tonumber(wndHandler:GetAmount() or 0)))
	self:RefreshBoundCredd() -- Will validate the buttons
end

function CheebaCREDD:OnGenerateTooltipFullStats(wndHandler, wndControl, eType, nX, nY) -- Same as from Commodity Exchange
	...
	local nLastCount = 0
	local wndFullStats = wndHandler:LoadTooltipForm("CheebaCREDD.xml", "FullStatsFrame", self)
	for nRowIdx = 1, 3 do
		local strBuy = ""
...
function CheebaCREDD:OnMoreMarketStatsMouseExit(wndHandler, wndControl)
...
function CheebaCREDD:OnConfirmationClose(wndHandler, wndControl)
	self.tWindowMap["ConfirmationBlocker"]:Show(false)
end

function CheebaCREDD:OnCreddTransactionBtn(wndHandler, wndControl, eMouseButton)
	wndHandler:SetFocus()
...
function CheebaCREDD:OnCREDDExchangeOrderSubmitted(wndHandler, wndControl)
...
function CheebaCREDD:OnCREDDExchangeOperationResults(eOperationType, eResult)
...
function CheebaCREDD:OnHidePostResultNotification() -- Both Timer and Mouse Click
...
function CheebaCREDD:OnCREDDOperationHistoryResults(tHistory)
...
function CheebaCREDD:HelperLogConvertToTimeString(nDays)
	local tTimeData =
	{
		["name"]	= "",
		["count"]	= nil,
...
local CheebaCREDDInst = CheebaCREDD:new()
CheebaCREDDInst:Init()
